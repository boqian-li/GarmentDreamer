...
from config_1d_uncondition import TrainingConfig
import os
...
def evaluate(config, iter_, unet, noise_scheduler, device):
    unet.eval()
        
    generator = torch.manual_seed(0) #cpu
    latents = torch.randn(
    (config.eval_batch_size, unet.config.in_channels, config.input_size),
    generator=generator,
    ).to(device)
    zipper = Zipper()
    ...
    os.makedirs(os.path.join('.', f'pretrained_unet'), exist_ok=True)   
    # 1. save unet model
    unet.save_pretrained(os.path.join('.', f'pretrained_unet/pretrained_unet_iter_{iter_}'))

    # 2. eval and save results
...
def train_loop(config, unet, noise_scheduler, optimizer, train_dataloader, lr_scheduler):
    ...
    for iter_ in range(config.total_iters):
        ...
        with accelerator.accumulate(unet):
            ...
            optimizer.zero_grad()
            noise_pred = unet(noisy_z, timesteps, return_dict=False)[0]
        ...
        if accelerator.is_main_process:
            if iter_ % config.predict_iter == 0 or iter_ == config.total_iters - 1:
                ...
                evaluate(config, iter_, accelerator.unwrap_model(unet), noise_scheduler, device=accelerator.device)
...
train_dataset = MyDataset(dict_ls, s="1d")
...
# )
# unet =  UNet1DModel(sample_size=config.input_size, in_channels = 1, out_channels=1,down_block_types =
...
unet = UNet1DModel(
    sample_size=config.input_size,  # the target image resolution
    in_channels=1,  # the number of input channels
    out_channels=1,  # the number of output channels
    extra_in_channels=16,
    block_out_channels=(32, 32, 64),  # the number of output channels for each UNet block
