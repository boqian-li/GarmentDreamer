...
for mesh_path in progress_bar(meshes_paths):
    ...
    pcd = get_tensor_pcd_from_o3d(pcd_o3d)[:, :3] # only xyz
    ...
    coords, labels, gradients = compute_udf_from_mesh(
        ...
        use_cuda=False
    ) # 用于decode的query points，values and gradients 
        # 在mesh表面 采集num_queries_on_surface个点， 周围采集num_queries_per_std个
        # 周围采集的query points限制 udf value <= 0.1, 所有query points都限制在（1， 1， 1）（-1， -1， -1）立方体内
        # 我处理的时候就全部normalize到单位球(+1e-3)内
